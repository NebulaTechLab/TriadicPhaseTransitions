...
# -------------------------
plt.figure()
plt.plot(t_fine, psi_exact, 'k--', label=r'Theory: $(Tc - T)^{3/2}$')
for N in system_sizes:
    plt.scatter(temperatures, results[N]['psi'], color=colors[N], edgecolor='k', s=25, alpha=0.8, label=f'M Carlo N = {N}')
plt.axvline(Tc, color='crimson', linestyle=':', lw=2, label=f'$Tc = {Tc:.1f}$')
plt.xlabel('Temperature ($T$)', fontsize=11)
plt.ylabel(r'$\Psi_{\mathrm{form}} \equiv \langle \varphi_i \varphi_j \varphi_k \rangle$', fontsize=11)
plt.ylim(-0.05, 1.05)
...
plt.xlabel(r'Conjugate Field ($h_3$)', fontsize=11)
...
axes[0, 0].plot(t_fine, psi_exact, 'k--', label=r'Theory: $(Tc - T)^{3/2}$')
for N in system_sizes:
...
axes[1, 0].set_title(r'(c) Equation of State at $T=Tc$')
